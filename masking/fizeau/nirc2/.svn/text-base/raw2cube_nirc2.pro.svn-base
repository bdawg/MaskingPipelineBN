...
;           Removed explicit binsize in sky subtract (now adaptive) PGT 30Apr04
;           When trimming array sq, grab brightest pix in cases where 
;              size explicitely passed, otherwise use array center  PGT 31Apr04

; !!! OBSOLETE _ NEED TO ADD CHANGES FROM RAW2CUBE_DITHER !!!
...
; %%%%% Populate hlog data structure.
  ...
  hlog.ra[i]         =headinfo.ra
  hlog.dec[i]        =headinfo.dec
  hlog.equinox[i]    =headinfo.equinox
...
; %%%%% Optionally trim array square
  if (keyword_set(setsquare) eq 1) then begin
     if (setsquare gt 1) then begin
        newdimxy=setsquare 
        im=grabnxn(im,newdimxy,x=xpeak,y=ypeak)
     endif else begin
        newdimxy=min([dimx,dimy])
...
;
; ??? PUt code here to check (uniq) that header info
; is all the same (eg same filter etc)
; Then collapse header info into single set for output to script
; ????
;
