function sigma_filter_nirc2, image, box_width, N_SIGMA=Nsigma, ALL_PIXELS=all,   $
					ITERATE=iterate, MONITOR=monitor,  $
					KEEP_OUTLIERS=keep, RADIUS=radius, $
		N_CHANGE=nchange, VARIANCE_IMAGE=imvar, DEVIATION_IMAGE=imdev,threshold=threshold,BAD_PIXELS=wbad
;+
; NAME:
;	SIGMA_FILTER
; PURPOSE:
...
;	THRESHOLD= only bother with bright cosmics (above thresh*stdev+mean)
; Optional Outputs:
...
;       BAD_PIXELS= the locations of the bad pixels/cosmic rays found.
; CALLS:
...
;       added threshold keyword to stop it flagging noise   PGT 11Mar04
;-
	...
	if keyword_set( keep )  then  wok = where( imdev GE imvar, nok ,complement=wbad) $
				else  wok = where( imdev LT imvar, nok ,complement=wbad)

	if keyword_set( threshold )  then begin 
          sd=stdev(image,mn)
	  wok = where( imdev LT imvar OR image LT mn+threshold*sd, nok ,complement=wbad)
        endif 

	npix = N_elements( image )
	nchange = npix - nok
	if keyword_set( monitor ) then $
		print, nchange, Nsigma, $
			FORM="('Cosmic Ray Cleaning: ',I8,'  pixels replaced, N_sigma=',F3.1)"
	...
	if keyword_set( iterate ) AND (Nsigma GE 2) then begin
	    ...
	    return, sigma_filter_nirc2( mean, box_width, N_SIGMA=Nsigma, ALL=all,$
					KEEP=keep, ITER=iterate, MONIT=monitor,threshold=threshold )
	   endif
