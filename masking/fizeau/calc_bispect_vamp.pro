...
;singlesave:    If set, this pro just does the first cube, saves out
;               the ps and stops. This is designed for template adjustment.

pro calc_bispect_vamp, cubeinfo_file,  root_dir=root_dir, ddir = ddir,  reset = reset,  plotall =plotall,  nosave = nosave, $
                  coherent_vis = coherent_vis,  coherent_bs = coherent_bs,  window_type = window_type, add_noise = add_noise, $
                  window_size = window_size, mf_file1 = mf_file1, mf_file2=mf_file2,  median_sub = median_sub,  n_blocks = n_blocks, $
                  subtract_bs_bias = subtract_bs_bias, window_mult=window_mult, special=special, cav=cav, rebin=rebin,disp_windowing=disp_windowing, $
                  all_ps=all_ps, singlesave=singlesave

; Compatibility for single mf file
if not keyword_set(mf_file2) then begin
   print,'Using a single mf file'
   mf_file2=mf_file1
endif else begin
   print,'Using two mf files'
endelse
...
restore,  root_dir + '/templates/' + mf_file1
...
for i=0, n_runs-1 do begin
   ...
   if (chanState eq 0) or (chanState eq 1) then restore,  root_dir + '/templates/' + mf_file1
   if (chanState eq 2) or (chanState eq 3) then restore,  root_dir + '/templates/' + mf_file2
   chanState += 1
   if chanState eq 4 then chanState = 0
    ...
    if (keyword_set(nosave) eq 0) then begin
        ;;;if (i eq 2) then bs_names =  'bs'+olog.cube_fname[i,1]+ special+'.idlvar' $
        ...
        save, u, v, v2, v2_sig, v2_cor, v2_cov, bs, cp, bs_var, cp_sig, bs_cov, bs_v2_cov,avar,err_avar,phs_v2corr, $
          mf_file1, cp_cov,cp_var, bs_all, v2_all, cvis_all, ps, $
          filename = analysis_dir + bs_names[i]
        plog =  {mf_file:mf_file1,  window_size:window_size,  coherent_vis:coherent_vis,  $
...
if keyword_set(singlesave) then begin
   print,'Saving 2 power spectrums and exiting.'
   if (chanState eq 1) then save,ps,file='ps_save.idlvar'
   if (chanState eq 3) then begin
      save,ps,file='ps_save2.idlvar'
      break
   endif
endif
