;+
; sky_subtract_vampires,image,radius,cent=cent,trim=trim,stats=stats,plotit=plotit,
...
; Modified version for VAMPIRES, BN August 2013
; Masks out *both* speckles, and smooths more to find peak.

pro sky_subtract_vampires,image1,radius,cent=cent,trim=trim,plotit=plotit,printit=printit,$
	binsize=binsize, stats=stats,bad_mask=bad_mask,subtr=subtr,submean=submean
;	
...
if (keyword_set(submean) eq 0) then submean=0
...
bmask=1.-mask
testim2=testim*bmask
locate_peak,smooth(testim2,10),xpeak,ypeak
mask2=fltarr(x,y)
make_circle,mask2,x/2,y/2,radius
mask2=shift(mask2,xpeak-x/2,ypeak-y/2)
mask=mask+mask2

index=where(mask eq 0 and bad_mask eq 0)
if (keyword_set(binsize) eq 0) then binsize=stdev(image(index))/20.
;binsize=0. ; Seems to work better if plothist finds a binsize (at least for VAMPRES).
plothist,image(index),x1,v2,/noplot;,bin=binsize,/noplot
imagenoise=sky_noise(image(index),imagelevel)  ;**** PGT Bugfix. Moved line to here ...

b2=[total(v2)*binsize,imagelevel,imagenoise]
if submean eq 0 then begin
...
if submean eq 1 then begin
    bglevel=mean(image(index))
    b2[1]=bglevel
    if plotit eq 1 then begin	
    	plot,x1,v2,tit="Sky Subtraction (mean)",xr=[b2(1)-3*b2(2),b2(1)+3*b2(2)]
    	oplot,[bglevel,bglevel],[0,1e4],linestyle=2
    endif
endif
...
if (subtr lt 1.0) then begin    
  ...
  print,'Sky subtraction value: ',stats[1]
endif
