...
1: BEGIN
	...
	prof1 = float( data )/maxd
	wp = where( prof1 LT 0.5, nw )
	...
	;	if (!DEBUG GT 2) then stop
		...
		if (prof1(mp-1) GE 1.e-4) OR $
		   (prof1(mp+1) GE 1.e-4) then begin

			non_Lin_Lsq, indgen( N_elements( prof1 ) ), prof1, $
