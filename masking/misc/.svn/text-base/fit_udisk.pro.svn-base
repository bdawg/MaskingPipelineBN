; This routine will fit a uniform Disk model to a set of data.
...
;
pro fit_udisk,sfu,vis,err,result=result,sigma=sigma,index=index, $
...
if (keyword_set(guess) eq 0) then guess=[mas2rad(0.0),1.0]
...
expr='((2*abs(beselj(3.14159*p(0)*X,1))>1e-6) / ((3.14159*p(0)*X)>1e-6))^'+power+' * p(1) + p(2)'
